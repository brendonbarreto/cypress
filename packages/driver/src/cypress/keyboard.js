...
var $Keyboard = {
  ...
  specialChars: {
    ...
    "{leftarrow}"(el, options) {
      ...
      return this.ensureKey(el, null, options, () => $selection.moveCursorLeft(el));
    },
    ...
    "{rightarrow}"(el, options) {
      ...
      return this.ensureKey(el, null, options, () => $selection.moveCursorRight(el));
    },
    ...
    "{uparrow}"(el, options) {
      ...
      return this.ensureKey(el, null, options, () => $selection.moveCursorUp(el));
    },
    ...
    "{downarrow}"(el, options) {
      ...
      return this.ensureKey(el, null, options, () => $selection.moveCursorDown(el));
    }
  },
  ...
  type(options = {}) {
    ...
    const keys = options.chars.split(charsBetweenCurlyBracesRe).map(function(chars) {
  ...
  typeChars(el, chars, options) {
    ...
    switch (false) {
      ...
      case !charsBetweenCurlyBracesRe.test(chars):
        ...
        //# char or modifier
        var allChars = _.keys(this.specialChars).concat(_.keys(this.modifierChars)).join(", ");
  ...
  simulateKey(el, eventType, key, options) {
    ...
    if (options[eventType] === false) { return true; }
    ...
    if (options.onBeforeEvent.apply(this, args) === false) { return; }
  ...
  isSpecialChar(chars) {
    return !!this.specialChars[chars];
  },
  ...
  handleModifier(el, chars, options) {
    ...
    if (this.modifiers[modifier]) { return; }
  ...
  activeModifiers() {
    return _.reduce(this.modifiers, function(memo, isActivated, modifier) {
      if (isActivated) { memo.push(modifier); }
      return memo;
    }
    , []);
  },
